...
const bannedWords = ["sex", "porn", "kill", "drugs", "shit", "fuck", "black"]
...
const fetchQuote = async () => {
    ...
    try {
        ...
        while (!cleanQuote) {
            ...
            const data = await response.json()
            const quote = data.quote.toLowerCase()
            const containsBadWord = bannedWords.some(word => quote.includes(word))
            if (!containsBadWord) {
                quoteEl.textContent = `"${data.quote}"`
                cleanQuote = true
                console.log("clean quote found")
...
document.querySelector(".generator").addEventListener("click", fetchQuote)